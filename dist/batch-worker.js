...
exports.startBatchWorker = startBatchWorker;
exports.stopBatchWorker = stopBatchWorker;
...
const rateLimiter = new token_bucket_1.TokenBucket(MAX_TX_PER_3S, MAX_TX_PER_3S / 3000);
...
async function collectBatch() {
    ...
    try {
        const result = await (0, db_1.getPool)().query(`WITH to_claim AS (
        SELECT id
        FROM publish_jobs
        WHERE status = 'queued'
      ...
      RETURNING
        pj.id, pj.job_id, pj.record_hash, pj.processing_batch_id, pj.batch_seq, pj.created_at`, [batchId, MAX_BATCH_SIZE]);
...
async function getActiveBatch() {
    const result = await (0, db_1.getPool)().query(`SELECT processing_batch_id
     FROM publish_jobs
 ...
 */
async function claimNextJob(batchId) {
    const result = await (0, db_1.getPool)().query(`WITH next_job AS (
      SELECT id
      FROM publish_jobs
    ...
    RETURNING pj.id, pj.job_id, pj.record_hash, pj.batch_seq`, [batchId]);
...
async function processJob(job) {
    ...
    try {
       ...
       WHERE id = $3`, ["BROADCAST_ERROR", error.message, job.id]);
    }
}
/**
 * Broadcaster: Continuously process batches with rate limiting
 */
async function broadcasterLoop() {
...
async function unstickJobs() {
    const TTL_MINUTES = 2;
    const result = await (0, db_1.getPool)().query(`UPDATE publish_jobs
 ...
 * Stop batch worker
 */
async function stopBatchWorker() {
...
//# sourceMappingURL=batch-worker.js.map